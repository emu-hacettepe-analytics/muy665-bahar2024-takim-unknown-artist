...
file_path \<- "C:/Users/Pc/Desktop/2023KONUT.xlsx" data \<- read_excel(file_path)

total_sales_2022 \<- sum(data\[\[2\]\], na.rm = TRUE) total_sales_2023 \<- sum(data\[\[3\]\], na.rm = TRUE)
...
```{r}
{library(readxl)} library(dplyr) library(ggplot2) library(tidyr) library(scales)
