...
# Deprem etkilenen illeri belirleyin
affected_cities <- c("ADANA", "ADIYAMAN", "DIYARBAKIR", "GAZIANTEP", "HATAY", "KILIS", "MALATYA", "OSMANIYE", "SANLIURFA", "KAHRAMANMARAS", "ELAZIG")

# Deprem etkilenen illeri filtreleyin
data_filtered <- data %>% filter(SEHIR %in% affected_cities)

# 2022 ve 2023 yıllarındaki konut satışlarındaki yüzdelik değişimi hesaplayın
...
# Bölgesel konut satış oranlarının hesaplanması
bolge_satis <- konut_data %>%
  group_by(BOLGE) %>%
  summarise(Satis_2022 = sum(`2022`),
            Satis_2023 = sum(`2023`)) %>%
  mutate(Oran_2022_2023 = (( Satis_2022 / Satis_2023)-1) * 100)
